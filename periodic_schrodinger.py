...
RHS = (eye(N**2, format="csr") + dt/2 * L)@u_now + dt*nonlinear_func(u_now)
...
for k in tqdm(range(1,N_t-1), desc="Evolving SE"):
    ...
    RHS = (eye(N**2, format="csr") + 1j*dt/2 * L)@u_now + 1j*3/2 * dt * nonlinear_func(u_now) - 1j*dt/2 * nonlinear_func(u_prev)
